...
int main()
{
    ...
    char continuum_name[256];
    char line_name[256];
    char temp_str[256];

    int i, num_mc, nt;
    double tbeg, tend;
    int nbin; // bin number of lag distribution

    /* read parameter file */
    char param_name[] = "param";

    FILE *fptr = NULL;
    fptr = fopen(param_name, "r"); // read parameter file
    fscanf(fptr, "%s %s", &temp_str, &continuum_name);
    while (fgetc(fptr) != '\n' && !feof(fptr)) {}
    fscanf(fptr, "%s %s", &temp_str, &line_name);
    while (fgetc(fptr) != '\n' && !feof(fptr)) {}
    fscanf(fptr, "%s %lf", &temp_str, &tbeg);
    while (fgetc(fptr) != '\n' && !feof(fptr)) {}
    fscanf(fptr, "%s %lf", &temp_str, &tend);
    while (fgetc(fptr) != '\n' && !feof(fptr)) {}
    fscanf(fptr, "%s %i", &temp_str, &nt);
    while (fgetc(fptr) != '\n' && !feof(fptr)) {}
    fscanf(fptr, "%s %i", &temp_str, &nbin);
    while (fgetc(fptr) != '\n' && !feof(fptr)) {}
    fscanf(fptr, "%s %i", &temp_str, &num_mc);
    while (fgetc(fptr) != '\n' && !feof(fptr)) {}
    fclose(fptr);

    /* input lc names */
    //char continuum_name[] = "temp_con.txt";
    //char line_name[] = "temp_line.txt";
    ...
    //int num_mc = 1000, nt = 6001;
    //double tbeg = -10.0, tend = 50.0;
    //int nbin = 601; // bin number of lag distribution
    ...
    printf("output time array - begin:%.6f end:%.6f length:%i step:%.6f\n", 
    ...
    printf("rmax:%.6f tau_cent:%.6f tau_peak:%.6f\n", rmax, tau_cent, tau_peak);
    printf("\n");
    ...
    printf("percentile(15.85, 50, 84.15) of tau centroid: %.6f %.6f %.6f\n", lower_lim, median, upper_lim);
    printf("time lag (centroid): %.6f - %.6f + %.6f\n", tau_cent, tau_cent - lower_lim, upper_lim - tau_cent);
    printf("\n");
    ...
    printf("valid monte carlo simulation:%i\n", num_mc);
    printf("percentile(15.85, 50, 84.15) of tau peak: %.6f %.6f %.6f\n", lower_lim, median, upper_lim);
    printf("time lag (peak): %.6f - %.6f + %.6f\n", tau_peak, tau_peak - lower_lim, upper_lim - tau_peak);
